...
class Member:
    ...
    __slots__ = ('_roles', 'joined_at', 'premium_since', '_client_status',
                 'activities', 'guild', 'pending', 'nick', '_user', '_client')

    def __init__(self, *, data, guild, client):
        self._client = client
        self._user = client.store_user(data['user'])
        self.guild = guild
    ...
    def _from_message(cls, *, message, data):
        ...
        return cls(data=data, guild=message.guild, client=message._client)
    ...
    @classmethod
    def _try_upgrade(cls, *,  data, guild, client):
        # A User object with a 'member' key
        try:
            member_data = data.pop('member')
        except KeyError:
            return client.store_user(data)
        else:
            member_data['user'] = data
            return cls(data=member_data, guild=guild, client=client)

    @classmethod
    def _from_presence_update(cls, *, data, guild, client):
        clone = cls(data=data, guild=guild, client=client)
        to_return = cls(data=data, guild=guild, client=client)
    ...
    @property
    def voice(self):
        """Optional[:class:`VoiceState`]: Returns the member's current voice client."""
        return self.guild._voice_client_for(self._user.id)
    ...
    async def edit(self, *, reason=None, **fields):
        ...
        me = self._client.self_id == self.id
        payload = {}
        ...
        if suppress is not None:
            voice_client_payload = {
                'channel_id': self.voice.channel.id,
                'suppress': suppress,
            }

            if suppress or self.bot:
                voice_client_payload['request_to_speak_timestamp'] = None

            if me:
                await http.edit_my_voice_client(guild_id, voice_client_payload)
            else:
                if not suppress:
                    voice_client_payload['request_to_speak_timestamp'] = datetime.datetime.utcnow().isoformat()
                await http.edit_voice_client(guild_id, self.id, voice_client_payload)
    ...
    async def request_to_speak(self):
        ...
        if self._client.self_id != self.id:
            payload['suppress'] = False
            await self._client.http.edit_voice_client(self.guild.id, self.id, payload)
        else:
            await self._client.http.edit_my_voice_client(self.guild.id, payload)
    ...
    async def add_roles(self, *roles, reason=None, atomic=True):
        ...
        if not atomic:
            ...
            req = self._client.http.add_role
            guild_id = self.guild.id
            user_id = self.id
            for role in roles:
    ...
    async def remove_roles(self, *roles, reason=None, atomic=True):
        ...
        if not atomic:
            ...
            req = self._client.http.remove_role
            guild_id = self.guild.id
            user_id = self.id
            for role in roles:
                await req(guild_id, user_id, role.id, reason=reason)
