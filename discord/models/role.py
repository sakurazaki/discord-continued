...
class Role:
    ...
    __slots__ = ('id', 'name', '_permissions', '_colour', 'position',
                 'managed', 'mentionable', 'hoist', 'guild', 'tags', '_client')

    def __init__(self, *, guild, data, client):
        self.guild = guild
        self.id = int(data['id'])
        self._client = client
        self._update(data)
    ...
    async def edit(self, *, reason=None, **fields):
        ...
        data = await self._client.http.edit_role(self.guild.id, self.id, reason=reason, **payload)
        self._update(data)
    ...
    async def delete(self, *, reason=None):
        ...
        await self._client.http.delete_role(self.guild.id, self.id, reason=reason)
